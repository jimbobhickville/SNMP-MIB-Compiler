...
my $ITEM_IDENTIFIER2
  = '\b[a-z](?:[\-_]?[A-Za-z0-9])*\b'; # w/allow_underscore
    ...
    while (1) {
      ...
      if ($c eq '-') {
        ...
        COMM: while (1) {
          ...
          return ($PLUSINFINITY, $val)
            if $val eq 'PLUS-INFINITY';
      ...
      if ($c !~ m/[A-Za-z0-9]/o) {
        ...
        return ($$keywords{$val}, $val)
          if defined $$keywords{$val};
...
sub compile {
  ...
  if ($self->{'make_dump'}) {
    ...
    if (defined $fh) {
      print $fh
        "## Compiled by SNMP::MIB::Compiler version $VERSION\n"
        . "## Source: $filename\n"
        . "## Date: "
      ...
      croak
"Warning: can't create dump $outdir/$file$self->{'dumpext'}"
        . ": $!\n";
    }
  }
...
sub resolve_type {
  ...
  # SMIv1 type
  return $type
    if $type =~ m/^(IpAddress|Counter|Gauge|TimeTicks|Opaque)$/o;
  ...
  while ($value eq 'MODULE') {
    ...
    while ($value ne 'MODULE' && $token != $ASSIGNMENT) {
      if ($value eq 'MANDATORY-GROUPS') {
        ...
        if ($value eq 'SYNTAX') {
          my $type = $self->parse_type();
          $$data{'module'}{$name}{'object'}{$val}{'syntax'}
            = $type;
          (($token, $value) = $self->get_token()) || return;
        }
        ...
        if ($value eq 'MIN-ACCESS') {
          ...
          if (
            $value
            =~ m/^(read-(only|write|create)|not-accessible|
    ...
    while (defined $token
      ...
      while ($value eq 'VARIATION') {
        ...
        if ($value eq 'ACCESS') {
          ...
          if (
            $value
            =~ m/^(not-implemented|accessible-for-notify|
...
sub import_modules {
  ...
  for my $k (keys %{ $self->{'imports'} }) {
    ...
    $mib->{'allow_underscore'} = $self->{'allow_underscore'};
    $mib->{'allow_lowcase_hstrings'}
      = $self->{'allow_lowcase_hstrings'};
    $mib->{'allow_lowcase_bstrings'}
      = $self->{'allow_lowcase_bstrings'};
